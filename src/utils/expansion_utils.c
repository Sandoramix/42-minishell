...
/*   Updated: 2024/06/19 21:24:45 by odudniak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	char **arg, int *dollar_idx, int *end_idx)
{
	...
	variable = str_substr(*arg, *dollar_idx + 1, *end_idx);
	if (!variable)
		return (pf_errcode(E_MALLOC), NULL);
	dbg_printf(CGRAY"\t\tVariable to search: [%s]\n"CR, variable);
	if (str_equals(variable, "?"))
		return (free(variable),
			expand_statuscode(mshell, arg, dollar_idx, end_idx));
	env = lst_findbykey_str(mshell->env, variable);
	free(variable);
	if (!env)
		return (expand_empty(arg, dollar_idx, end_idx));
	dbg_printf(CGREEN"\tVariable found with value: [%s]\n"CR, env->val);
	*arg = str_replace_from_to(*arg, *dollar_idx, *end_idx, env->val);
	if (!(*arg))
		return (pf_errcode(E_MALLOC), NULL);
	*dollar_idx = str_idxofchar(*arg, '$');
	return (*arg);
